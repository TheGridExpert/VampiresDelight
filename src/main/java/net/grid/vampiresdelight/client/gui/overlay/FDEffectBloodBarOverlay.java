package net.grid.vampiresdelight.client.gui.overlay;
...
public class FDEffectBloodBarOverlay extends GuiComponent implements IGuiOverlay {
    private final ResourceLocation icons = new ResourceLocation(VampiresDelight.MODID + ":textures/gui/icons.png");
            ...
            if (this.mc.gameMode.hasExperience() && this.mc.player.isAlive()) {
                VampirePlayer.getOpt(this.mc.player).map(VampirePlayer::getBloodStats).ifPresent(stats -> {
                            ...
                            for (int i = 0; i < 10; ++i) {
                                ...
                                // Draw Background
                                blit(poseStack, x, top, 0, idx <= maxBlood2 ? 9 : 0, 9, 9);

                                if (idx < blood) {
                                    blit(poseStack, x, top, 9, idx < blood2 ? 9 : 0, 9, 9);
                                    if (idx == blood2) {
                                        blit(poseStack, x, top, 18, 9, 9, 9);
                                    }
                                } else if (idx == blood) {
                                    blit(poseStack, x, top, 18, 0, 9, 9);
                                }
                            }
